...
def main() -> int:
    """Update implementation status and dbt usage sections."""
    text = README_PATH.read_text(encoding="utf-8")
    text = replace_section(text, "## Project status", STATUS)
    if "## dbt transformations and data quality" in text:
        text = replace_section(text, "## dbt transformations and data quality", DBT)
    else:
        text = insert_before(text, "## Data and credentials", DBT)
    with README_PATH.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(text)
    print("README updated with the dbt transformation layer.")
    return 0
