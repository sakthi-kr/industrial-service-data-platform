...
EXPECTED_MART_MODELS = 5
EXPECTED_SINGULAR_TESTS = 13
...
def declared_source_pairs() -> set[tuple[str, str]]:
    """Read source/table pairs from the tracked dbt source YAML files."""
    pairs: set[tuple[str, str]] = set()

    for path in sorted((DBT_ROOT / "models").rglob("*sources.yml")):
        ...
        for raw_line in path.read_text(encoding="utf-8").splitlines():
            ...
            if stripped.startswith("- name:"):
                ...
                if indent <= 4:
                    source_name = name
                    in_tables = False
                elif in_tables and source_name is not None:
                    pairs.add((source_name, name))
...
def validate_model_references() -> None:
    ...
    sources = declared_source_pairs()
    missing_refs: list[str] = []
    missing_sources: list[str] = []

    for path in model_paths:
        text = path.read_text(encoding="utf-8")
        for reference in re.findall(r"ref\(['\"]([^'\"]+)['\"]\)", text):
...
def main() -> int:
    ...
    required = [
        ...
        DBT_ROOT / "models/staging/_enrichment_sources.yml",
        DBT_ROOT / "models/core/_core_models.yml",
        DBT_ROOT / "models/marts/_marts.yml",
        DBT_ROOT / "models/marts/mart_technician_note_enrichment.sql",
